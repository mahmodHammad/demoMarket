...
const FilterAndSort: React.FC<Props> = ({ sorting, filtering, handleSort, handleFilter, filterValues, sortValues }) => {
				...
				<Item sx={{ mr: 6 }}>
					...
					<Menu
						...
						{Object.entries(filterValues).map(([id, type]) => (
							<MenuItem key={id} data-my-value={id} onClick={() => handleFilterChange(id)}>
								{type?.color ? (
									<Text
										variant="caption"
										sx={{
				...
				<Box
					...
					<Menu
						...
						{Object.entries(sortValues).map(([id, type]) => {
							return (
								<MenuItem key={id} data-my-value={id} onClick={() => handleSortChange(id)}>
									{type}
								</MenuItem>
							);
						})}
